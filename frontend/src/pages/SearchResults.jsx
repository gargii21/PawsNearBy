...
//         if (list.length === 0) {
//           list = [
//             { prId: 1, daycare_name: "Paws and Claws", service: "Pet Sitting", lat: 28.61, lng: 77.20, city: "Delhi", fees: 500, rating: 4.5 },
//             { prId: 2, daycare_name: "Happy Paws Pet Daycare", service: "Dog Walking", lat: 19.07, lng: 72.87, city: "Mumbai", fees: 400, rating: 4.2 },
//           ];
//         }
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        ...
        // Set the profiles from the backend
        setProfileTest(list);

      } catch (error) {
        console.log("Error fetching profiles. Backend might be down.");
        setProfileTest([]);  // In case of an error, just set the profiles to an empty array
      } finally {
        setLoading(false);
      }
    };
